...
The test uses actual separate PurrNet processes and checks roles across scene changes, the selected case, competing team requests, chunked PNG wheel uploads, finish-line collisions, destroyed-car cleanup, flips, cancellation, role/evidence rejection, objection pause/resume, all speaking boundaries, and the judge's verdict. Clock boundaries are accelerated by the development-only harness; production gameplay always uses the real 20-minute schedule. The harness is only instantiated in the generated test scene.

Drawings are capped at 2 MiB and sent as ordered 8 KiB chunks. This avoids WebTransport's 64 KiB packet-buffer limit; a single large RPC fails even with the fragmentation attribute. The server accepts only contiguous chunks for one upload per player, bounds its memory, expires incomplete uploads, and validates the completed drawing and team reservation before creating a car.

## Final validation, September 16, 2026

The isolated Unity build passed the rules/assets validator. The six-process integration run logged `COURT NETWORK SMOKE PASSED` on the host and `COURT CLIENT SMOKE PASSED` on all five controllers. This run used a noisy 256 × 256 PNG of roughly 148 KB, exercising multiple upload chunks rather than a trivially small drawing. All seven turn transitions and the final networked verdict passed. No runtime exceptions occurred in the successful run.

The first WebGL development build compiled successfully and was opened in the browser: guest login, private lobby creation, and the three-participant requirement message worked without browser console errors. The follow-up fixes also persist the network manager in the CaseSelection scene and theme PurrUI's procedural panels, which are separate from ordinary Unity Image backgrounds.

See [WebGLSubmission.md](WebGLSubmission.md) for the submission build command and upload instructions.
